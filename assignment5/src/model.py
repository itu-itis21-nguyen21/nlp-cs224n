...
class DownProjectBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.C = nn.init.xavier_uniform_(torch.empty(1, config.bottleneck_dim, config.n_embd)).to(device)
        self.mlp = nn.Sequential(
            nn.Linear(config.n_embd, 4 * config.n_embd),
            nn.GELU(),
            nn.Linear(4 * config.n_embd, config.n_embd),
    ...
    def forward(self, x_input):
        ...
        mid = self.C + self.attn(x_input, self.ln1(self.C))
        mid = mid + self.mlp(self.ln2(self.C))
        return mid
        ### END YOUR CODE
