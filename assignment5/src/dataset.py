...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        document = document[:random.randint(3, int(self.block_size*7/8)-1)]
        
        # 2. Break document into 3 substrings
        document_length = len(document)
        mask_length = random.randint(1, document_length//2+1)
        prefix_length = random.randint(1, document_length - mask_length)
        prefix = document[:prefix_length]
        masked_content = document[prefix_length:mask_length+prefix_length]
        suffix = document[mask_length+prefix_length:]
        ...
        # 5. Encode input and output as Long tensors
        x = torch.tensor([self.stoi[_] for _ in x], dtype=torch.long)
        y = torch.tensor([self.stoi[_] for _ in y], dtype=torch.long)
