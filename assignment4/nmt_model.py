...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # 2. Compute enc_hiddens, last_hidden, and last_cell
        packed_X = pack_padded_sequence(X, source_lengths)
        packed_enc_hiddens, (last_hidden, last_cell) = self.encoder(packed_X)
        enc_hiddens, _ = pad_packed_sequence(packed_enc_hiddens, batch_first=True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 3. Use torch.split to iterate over Y
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 1. Apply the decoder to obtain the new dec_state
        dec_state = self.decoder(Ybar_t, dec_state)

        # 2. Split dec_state
        (dec_hidden, dec_cell) = dec_state

        # 3. Compute e_t using torch.bmm
        ...
        # 2. Compute attention output vector a_t
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)

        # 3. Concatenate dec_hidden with a_t
        U_t = torch.cat((a_t, dec_hidden), dim=1)

        # 4. Apply combined output projection layer to U_t
        V_t = self.combined_output_projection(U_t)

        # 5. Compute combined-output O_t
