...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        w_flatten = w.flatten()
        x = torch.index_select(self.embeddings, 0, w_flatten).view(w.shape[0], self.n_features * self.embed_size)
    ...
    def forward(self, w):
        ...
        h = nn.ReLU()
        output_hidden = h(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        if self.training:
            output_hidden = self.dropout(output_hidden)
        logits = torch.matmul(output_hidden, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
